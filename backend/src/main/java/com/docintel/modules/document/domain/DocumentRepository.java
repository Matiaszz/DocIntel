...
public interface DocumentRepository extends JpaRepository<Document, UUID> {
    ...
    @Query("SELECT DISTINCT d FROM Document d LEFT JOIN Folder f ON d.folder = f LEFT JOIN FolderPermission fp ON fp.folder = f " +
           "WHERE (d.owner.id = :userId OR f.owner.id = :userId OR (fp.user.id = :userId AND fp.inviteStatus = com.docintel.modules.folder.domain.enums.FolderInviteStatus.ACCEPTED) OR f.folderVisibility = 'PUBLIC' " +
    ...
    @Query("SELECT DISTINCT d FROM Document d LEFT JOIN Folder f ON d.folder = f LEFT JOIN FolderPermission fp ON fp.folder = f " +
           "WHERE (d.owner.id = :userId OR f.owner.id = :userId OR (fp.user.id = :userId AND fp.inviteStatus = com.docintel.modules.folder.domain.enums.FolderInviteStatus.ACCEPTED) OR f.folderVisibility = 'PUBLIC' OR (d.folder IS NULL AND d.owner.id = :userId)) " +
